...
export default function PhenomenonPage() {
  ...
  async function generateContent() {
    ...
    setGenerating(true)
    try {
      const res = await fetch(`/api/phenomena/${phenomenon.slug}`, {
        method: 'POST',
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-950">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {needsContent && (
            <div className="mb-8 p-4 bg-purple-900/20 border border-purple-800 rounded-lg flex items-center justify-between">
              <div>
                <p className="text-purple-300 font-medium">AI content not yet generated</p>
                <p className="text-purple-400/70 text-sm">Click to generate detailed descriptions, history, and characteristics.</p>
              </div>
              <button
                onClick={generateContent}
                disabled={generating}
                className="px-4 py-2 bg-purple-600 hover:bg-purple-500 disabled:bg-purple-800 text-white rounded-lg flex items-center gap-2 transition-colors"
              >
                <RefreshCw className={classNames('w-4 h-4', generating && 'animate-spin')} />
          ...
          {activeTab === 'history' && (
            <div className="max-w-3xl space-y-8">
              ...
              {/* AI Attribution */}
              {phenomenon.ai_generated_at && (
                <p className="text-xs text-gray-500 mt-8">
                  Content generated by AI on {new Date(phenomenon.ai_generated_at).toLocaleDateString()}.
                  AI-generated content is for informational purposes and may contain inaccuracies.
                </p>
              )}
            </div>
          )}
