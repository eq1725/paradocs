        ...
        {!isLoggedIn && !loading ? (
          ...
          <div className={activeTab === 'story' ? 'pb-20' : ''}>
            {activeTab === 'story' && (() => {
              ...
              // preserved for all other entry points. Detection is
              // URL-param-only; no persistence.
              var fromParam = router.query.from
              var elevateDelta = fromParam === 'digest' || fromParam === 'push'
              return (
                <div style={{ minHeight: 'calc(100dvh - 200px)' }}>
                  {/* T1.11 portal target for the elevated delta line.
                      YourSignalTab renders SinceLastVisitLine into this
                      container via createPortal when elevateDelta is on. */}
                  {elevateDelta && (
                    <div
...
function YourSignalTab() {
  ...
  // cards (cluster · fingerprint · context · peer_questions) in a
  // snap-x mandatory horizontal scroller with edge-peek and pagination
  // dots instead of the single algorithmically-picked hero. The
  // MoreSignalsAccordion is then redundant and hidden.
  var heroCarouselEnabled = getFeatureFlag('signal-hero-pick-strategy') === 'carousel'

  // E0.6 — Free users get full AI Story analysis (RADAR + Cards 1, 2,
  // 3 + Ask) on their FIRST submitted experience. Subsequent
  // experiences get only the cheap deterministic cards (Cards 1 + 2);
  // Card 3 (the Sonnet-derived "context" / Did You Know) renders a
  // locked-state placeholder with an upgrade CTA. Basic/Pro users get
  // full AI on every experience. See docs/TIER_DESIGN_V2.md.
  ...
  return (
    <div className="space-y-4 max-w-3xl mx-auto">
      ...
      {/* V10.9 — "Since you last visited" delta line. Single most
          ...
          T1.11 — when this visit was referred from a digest email or
          push notification, render the same line into the elevated
          slot above RADAR via React portal. Otherwise render in
          place. URL detection only; no persistence. */}
      <DeltaLineSlot sinceLastVisit={data.since_last_visit} hasReport={true} />

      {/* T1.10 — carousel variant (flag-gated) renders all 4 cards in
          a snap-x horizontal scroller. Default behavior preserves the
          V10.9 single hero card.
          E0.6 — aiCardsLocked tells the renderer to swap the Sonnet
          context card for a locked-state upgrade CTA. */}
      {heroCarouselEnabled ? (
        <HeroCardCarousel
          data={data}
 ...
 * the line is portaled into the elevated slot above RADAR; otherwise
 * it renders inline (where it normally lives, above the hero card).
 * One-visit-only behavior — purely URL-driven, no persistence.
 *
 * Uses React Portal so YourSignalTab keeps owning the data fetch
 * without prop-drilling or state-lifting.
 */
function DeltaLineSlot(props: { sinceLastVisit: any; hasReport: boolean }) {
  ...
  useEffect(function () {
    ...
    // Defer until the next tick to let the parent's elevated slot mount.
...
function HeroCardSlot(props: { heroCard: { kind: string; data: any }; reportId: string; feedback: any; aiCardsLocked?: boolean }) {
  ...
  // beyond their first submitted experience. The locked-state IS the
  // conversion trigger surface — full attention, full-bleed card
  // estate with an "Upgrade to Basic" CTA where the insight would be.
 ...
 * (the "context" / Did You Know slot) shown to Free users beyond
 * their first submitted experience. Conversion trigger surface per
 * docs/TIER_DESIGN_V2.md § "Conversion trigger surfaces".
 *
 * Visual: same gradient envelope as the unlocked card, but the body
 * is a lock icon + value-prop one-liner + upgrade CTA. We don't blur
 * a fake insight (felt deceptive in panel review) — we show what the
 * paid version would unlock in plain language.
 */
...
/**
 * T1.10 — Hero card carousel.
 *
 * Renders all four Signal cards (cluster · fingerprint · context ·
 * peer_questions) in a snap-x mandatory horizontal scroller with
 * edge-peek + pagination dots. Flag-gated via signal-hero-pick-
 * strategy === 'carousel'. Replaces both the single HeroCardSlot
 * and the MoreSignalsAccordion when active.
 *
 * Each slide gets the same per-kind gradient/border as the hero slot
 * so visual identity is preserved. Cards without data are skipped
 * (their underlying components already handle empty states).
 *
 * Uses IntersectionObserver to track which slide is currently centered
 * for active-dot rendering. Falls back gracefully if IO is unavailable.
 */
function HeroCardCarousel(props: { data: any; reportId: string; feedback: any; aiCardsLocked?: boolean }) {
  ...
  // Build the slide list, skipping kinds with no usable data.
  // E0.6 — the 'context' slide swaps for a locked-state CTA on Free
  // users beyond their first submitted experience. Other slides
  // render normally (cluster + fingerprint are deterministic, no
  // Sonnet cost; peer_questions is cross-user aggregation, also free).
  ...
  if (props.aiCardsLocked) {
    // Always include the locked context slide so Free users always see
    // the upgrade trigger in the carousel, even if the unlocked card
    // would have been skipped for missing data.
  ...
  // Per-kind gradient/border — matches HeroCardSlot heroBg so visual
  // identity is consistent across the two render modes.
  var heroBg: Record<string, string> = {
    cluster:        'from-purple-900/30 via-purple-950/30 to-fuchsia-900/20 border-purple-600/40',
    fingerprint:    'from-cyan-900/25 via-purple-950/25 to-purple-900/30 border-cyan-600/30',
    context:        'from-indigo-900/30 via-purple-950/25 to-purple-900/30 border-indigo-500/40',
  ...
  // Track which slide is centered for active-dot rendering.
  useEffect(function () {
  ...
  if (slides.length === 0) {
    // No data at all — render the same empty-state ClusterCard the
    // single-hero path would render.
    return (
