...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // best-effort; on failure we fall back to 'free' + true (most-
  // permissive, free-tier-friendly).
  var tierName = 'free'
  try {
  ...
  try {
    ...
    // First submission iff the chronologically-first one is the focused report.
    if (subs.length > 0) {
  ...
  return res.status(200).json({
    ...
    // Free users beyond their first experience. See docs/TIER_DESIGN_V2.md
    // § "Free first-experience hook".
    tier_name: tierName,
...
async function loadPeers(svc: any, currentUserId: string, userReport: any) {
  try {
    ...
    // Paradocs user who could receive a DM. Filter at the API level
    // so the PeopleLikeYouCard never even sees them.
    var query = await svc.from('reports')
