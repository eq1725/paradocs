...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  var { data: video, error: videoErr } = await admin
    .from('report_videos')
    .select('id, report_id, user_id, status, storage_bucket, storage_path, mime_type, size_bytes, duration_sec, transcript, transcript_segments, transcript_lang, extracted_meta, uploaded_at, transcribe_attempts, transcribe_error, transcribed_at')
  ...
  return res.status(200).json({
    ...
    video: {
      ...
      transcribe_attempts: (video as any).transcribe_attempts || 0,
      transcribe_error: (video as any).transcribe_error || null,
      transcribed_at: (video as any).transcribed_at || null,
