...
var SYSTEM_PROMPT = ANTI_FABRICATION_PREAMBLE
  ...
  + '- JUDGMENT, NOT KEYWORDS. Many surface-mundane phrases ARE anomalous in context, and many surface-anomalous phrases are mundane. Read the whole account.\n'
  + '- KEEP (anomalous="yes") — UFO sightings, non-human entities, MISSING TIME / lost hours (even when phrased as "lost an hour driving" — these are documented abduction phenomena), apparitions, hauntings (even subtle — "chill" PAIRED with shadow figure / sensed presence / unexplained sound), poltergeists, witnessed phenomena, precognitive dreams that came true, telepathy, OBE, NDE, sleep paralysis WITH sensed presence/entity (not paralysis alone), cryptid sightings, witness sketches, synchronicity, manifestation experiences, lights with anomalous behavior (hover/split/silent flight).\n'
  + '- ARCHIVE (anomalous="no") — a SINGLE loud boom or vibration alone (mundane physical event — seismic/industrial/mechanical) UNLESS paired with lights/entity/missing time/traces; "saw a light" or "strange-looking person" with no behavior or encounter; "felt a chill" attributable to draft/AC with no paired phenomenon; vivid dream alone with no precognitive payoff or corroboration; third-person commentary on someone else\'s experience (YouTube reaction comment, news summary); Wikipedia-style phenomenon summaries; mundane hiking/navigation/dehydration; wildlife encounters (animal is the danger); perceptual-quirk explainers; platform/algorithm/media complaints; opinion/theory pieces, advice requests, product reviews; psychological/emotional change with no anomalous element; promotional content; single-word reactions.\n'
  + '- WORKED EXAMPLE: "Heard a loud boom that shook the hotel. Asked a neighbor; he said not a bomb. Maybe underground." → anomalous="no" conf 0.85. Single sound, witness offers mundane frame, no other indicator.\n'
  + '- WORKED EXAMPLE: "Heard a boom, then saw a glowing object lift off the ridge silently." → anomalous="yes" conf 0.9. The silent glowing object is the anomaly.\n'
  + '- WORKED EXAMPLE: "Lost an hour driving home with no memory of the road." → anomalous="yes" conf 0.85. Missing-time is a documented phenomenon.\n'
  + '- CONFIDENCE: 0.95-1.0 unambiguous; 0.80-0.94 confident-not-certain (the Sedona-boom belongs here ~0.85); 0.70-0.79 leaning sparingly; <0.70 uncertain → default anomalous="yes" so the gate does NOT fire.\n'
  + '- Be STRICT but FAIR. When the witness themselves offers a mundane explanation AND no other anomalous element appears, archive with confidence.\n\n'
  + 'HOOK RULES:\n'
