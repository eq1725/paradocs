#!/usr/bin/env tsx
/**
 * NDERF Mass Ingestion Runner (V11.17.13)
 *
 * Fork of scripts/nuforc-mass-ingest.ts adapted for NDERF (nderf.org).
 *
 * KEY DIFFERENCES vs NUFORC orchestrator:
 *   - Shards by archive URL (NDERF has ~4 archive pages), not by month.
 *   - Default concurrency=1: NDERF is a single academic-host site (Dr.
 ...
 *   - No shape-map needed: NDERF adapter already sets
 ...
 *   - No --strict-mode default: NDERF narratives are naturally
 *     1000-5000 chars; the base 200-char floor is irrelevant.
 *   - Adds HTTP-error-aware backoff: any 429/503 from NDERF triggers
 ...
 *   tsx scripts/nderf-mass-ingest.ts --dry-run
 *
 *   # Full corpus, polite single-worker (~1 req/sec), $500 cap
 *   tsx scripts/nderf-mass-ingest.ts \
 *     --target 10000 \
 *     --rate-limit-ms 1000 \
 *     --cost-cap 500
 *
 *   # Resume an interrupted run
 *   tsx scripts/nderf-mass-ingest.ts --resume
 *
 * ── Architecture ────────────────────────────────────────────────────
 *
 * Each shard = one NDERF archive URL. Worker fetches the archive's
 ...
 * Batch API (50% off).
 *
 * State persisted to outputs/nderf-mass-ingest-state.json. Resume-safe.
 *
 * ── Stop conditions ─────────────────────────────────────────────────
...
import { nderfAdapter } from '../src/lib/ingestion/adapters/nderf';
...
const NDERF_SOURCE_TYPE = 'nderf';
const STATE_FILE = path.resolve(process.cwd(), 'outputs/nderf-mass-ingest-state.json');
...
// Fallback archive list if --discover fails or is disabled.
// The orchestrator prefers DYNAMIC discovery via fetchArchiveList()
// which parses https://www.nderf.org/Archives/archivelist.htm and
// returns the full ~54 archives going back to 1998. This static list
// is only used if --no-discover is passed OR if discovery fails.
const FALLBACK_ARCHIVE_URLS = [
  'https://www.nderf.org/Archives/NDERF_NDEs.html',
  'https://www.nderf.org/Archives/exceptional.html',
];

// Canonical NDERF master index. Lists all date-range archive pages
// (1998-present). Refetched at orchestrator startup.
const NDERF_ARCHIVE_LIST_URL = 'https://www.nderf.org/Archives/archivelist.htm';

/**
 * Fetch the NDERF master archive index and extract all archive sub-page
 * URLs. The index is small (~50KB) and very stable; one fetch at start
 * of run gives us the full ~5000-case scope.
 *
 * Returns deduped, sorted-newest-first list.
 */
async function fetchArchiveList(): Promise<string[]> {
  try {
    const resp = await fetch(NDERF_ARCHIVE_LIST_URL, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; Paradocs-Archive/1.0; +https://www.discoverparadocs.com)',
      },
    });
    if (!resp.ok) {
      console.warn('[nderf-mass-ingest] archivelist.htm returned ' + resp.status + '; using fallback list');
      return FALLBACK_ARCHIVE_URLS.slice();
    }
    const html = await resp.text();
    // Extract all archive sub-page URLs. Match the patterns:
    //   /Archives/NDERF_NDEs.html
    //   /Archives/exceptional.html
    //   /Archives/2_X_YYYY.html  (date-range archives, e.g. 2_1_2025.html)
    //   /Archives/2_X_YYYY_Z.html (split-period archives, e.g. 2_6_2010_1.html)
    //   /Archives/2_YYYY.html  (year-only archives like 2_2003.html)
    //   /Archives/2_YYYY_YYYY.html (multi-year like 2_1998_2001.html)
    const archiveUrls = new Set<string>();
    const linkPattern = /href="(https?:\/\/www\.nderf\.org\/Archives\/[^"]+\.html?)"/gi;
    let m: RegExpExecArray | null;
    while ((m = linkPattern.exec(html)) !== null) {
      const url = m[1];
      // Skip the archivelist itself + obvious non-archive pages
      if (url.includes('archivelist.htm')) continue;
      archiveUrls.add(url);
    }
    if (archiveUrls.size === 0) {
      console.warn('[nderf-mass-ingest] archivelist.htm parsed but no archive URLs found; using fallback');
      return FALLBACK_ARCHIVE_URLS.slice();
    }
    // Sort: newest-first heuristic — archives with higher year-numbers first,
    // then alpha for the non-dated ones. This processes recent reports first
    // (richer/more-detailed) before old ones.
    const sorted = Array.from(archiveUrls).sort((a, b) => {
      // Extract year from URL if present (last 4-digit group)
      const ya = (a.match(/(\d{4})/g) || [])[0] || '9999'; // non-dated to top
      const yb = (b.match(/(\d{4})/g) || [])[0] || '9999';
      if (ya !== yb) return parseInt(yb) - parseInt(ya);
      return a.localeCompare(b);
    });
    return sorted;
  } catch (e: any) {
    console.warn('[nderf-mass-ingest] Could not fetch archivelist.htm: ' + (e?.message || e) + '; using fallback');
    return FALLBACK_ARCHIVE_URLS.slice();
  }
}
...
function parseArgs(): CliArgs {
  ...
  return {
    ...
    // If --archives is given, use it explicitly (skip auto-discover).
    // Otherwise use FALLBACK list; main() will replace with discovered list.
    archives: archivesArg ? archivesArg.split(',').map(s => s.trim()).filter(Boolean) : FALLBACK_ARCHIVE_URLS.slice(),
...
function loadState(): MassIngestState | null {
  ...
  try {
    ...
    console.error('[nderf-mass-ingest] Could not parse state file, ignoring:', (e as any)?.message);
    return null;
  }
}
...
// has experienceTypeSlug (NDERF/OBERF always set this), creates the
...
// once (~3 slugs total for NDERF), then reused across thousands of rows.
...
async function processShard(
  ...
  // 1. Scrape this archive's experiences via the adapter (targetArchives fast path)
  const scrapeResult = await nderfAdapter.scrape({
    target_archives: [archiveUrl],
  ...
  if (reportIds.length > 0) {
    for (let i = 0; i < reportIds.length; i += 500) {
      ...
      const { data } = await supabase
        ...
        .eq('source_type', NDERF_SOURCE_TYPE)
  ...
  for (const report of reports) {
    try {
      ...
      const insertData: Record<string, any> = {
        ...
        tags: report.tags || [],
        source_label: report.source_label || 'NDERF',
      ...
      result.errors++;
      console.warn('[nderf-mass-ingest] per-report error: ' + (perReportErr?.message || perReportErr));
    }
  }
...
function triggerBatchWorker(insertCount: number): void {
  ...
  console.log('[nderf-mass-ingest] Triggering batch worker (cumulative inserts: ' + insertCount + ')');
...
async function runWorkerPool(
  ...
  if (pendingShards.length === 0) {
    console.log('[nderf-mass-ingest] No pending shards. Run complete.');
    return;
  }
  ...
  async function worker(workerId: number): Promise<void> {
    while (!controller.abort) {
      ...
      console.log('[nderf-mass-ingest] Worker ' + workerId + ' starting archive: ' + shard.archiveUrl);
      const tStart = Date.now();
      try {
        const r = await processShard(supabase, shard.archiveUrl, args);
        ...
        console.log('[nderf-mass-ingest] Worker ' + workerId + ' completed ' + shard.archiveUrl + ' in ' + formatDuration(shard.ms) + ' — scraped=' + r.scraped + ' inserted=' + r.inserted + ' dup=' + r.duplicates + ' filtered=' + r.filtered);
        ...
        state.totals.errors++;
        saveState(state);
        console.error('[nderf-mass-ingest] Worker ' + workerId + ' shard ' + shard.archiveUrl + ' FAILED: ' + shard.error);
      }
    }
  }
...
async function main() {
  ...
  console.log('NDERF Mass Ingest — V11.17.13');
  console.log('Args: ' + JSON.stringify(args));

  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error('[nderf-mass-ingest] Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY. Source .env.local first.');
    process.exit(1);
  }
  if (!process.env.ANTHROPIC_API_KEY) {
    console.warn('[nderf-mass-ingest] WARNING: ANTHROPIC_API_KEY missing. Batch worker will fail when triggered.');
  }
  ...
  if (args.resume) {
    ...
    if (!loaded) {
      console.error('[nderf-mass-ingest] --resume requested but no state file at ' + STATE_FILE);
      process.exit(1);
    }
    state = loaded;
    state.args = args;
    ...
    console.log('[nderf-mass-ingest] Resuming: ' + completedCount + ' completed, ' + failedCount + ' failed, ' + state.totals.inserted + ' inserted so far');
    state.shards.forEach(s => { if (s.status === 'in_progress') s.status = 'pending'; });
  } else {
    state = emptyState(args);
    // V11.17.13 — Dynamic archive discovery. Unless --no-discover or
    // --archives was passed, fetch the NDERF master archive list to
    // build the shard set. This expands ~4 hardcoded archives to all
    // ~54 date-range archives (~5,000 cases total).
    let archives: string[];
    if (args.archivesExplicit || args.noDiscover) {
      archives = args.archives;
      console.log('[nderf-mass-ingest] Using explicit/fallback archive list (' + archives.length + ' archives)');
    } else {
      console.log('[nderf-mass-ingest] Discovering archives from ' + NDERF_ARCHIVE_LIST_URL + '...');
      archives = await fetchArchiveList();
      console.log('[nderf-mass-ingest] Discovered ' + archives.length + ' archive(s)');
    }
    state.shards = archives.map(archiveUrl => ({
      archiveUrl, status: 'pending', scraped: 0, inserted: 0, duplicates: 0, filtered: 0, errors: 0, ms: 0,
    }));
    console.log('[nderf-mass-ingest] Enumerated ' + state.shards.length + ' archive-shards');
    state.shards.slice(0, 10).forEach(s => console.log('  - ' + s.archiveUrl));
    if (state.shards.length > 10) {
      console.log('  ... + ' + (state.shards.length - 10) + ' more (truncated)');
    }
  }

  if (args.dryRun) {
    console.log('[nderf-mass-ingest] --dry-run; exiting');
    return;
  }

  state.totals.costUsd = await pollTodaysCost(supabase);
  console.log('[nderf-mass-ingest] Starting today\'s cost: $' + state.totals.costUsd.toFixed(2) + ' (cap: $' + args.costCapUsd.toFixed(2) + ')');
  ...
  process.on('SIGINT', () => {
    sigintCount++;
    if (sigintCount === 1) {
      console.log('\n[nderf-mass-ingest] Caught SIGINT, finishing in-flight shards and saving state. Press Ctrl+C again to force-exit.');
      controller.abort = true;
      controller.reason = 'SIGINT';
    } else {
      console.log('[nderf-mass-ingest] Force exit on second SIGINT.');
      saveState(state);
      process.exit(130);
    }
  });
  ...
  if (!args.noBatchTrigger && state.totals.inserted > state.lastBatchTriggerInserts) {
    console.log('\n[nderf-mass-ingest] Triggering final batch worker drain...');
    triggerBatchWorker(state.totals.inserted);
  }
}

main().catch(err => {
  console.error('[nderf-mass-ingest] Fatal:', err);
  process.exit(1);
});
