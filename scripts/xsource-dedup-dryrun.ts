...
const COSINE_MERGE = parseFloat(flag('--cosine-merge', '0.93'))
const COSINE_CLUSTER = parseFloat(flag('--cosine-cluster', '0.88'))
const MERGE_TEXT = parseFloat(flag('--merge-text', '0.50'))  // word-shingle Jaccard floor for "same account"
...
async function main() {
  ...
  const sb = makeSb()
  console.log('=== Cross-source dedup DRY RUN (V11.35) ===')
  console.log(`category=${CATEGORY} scan-limit=${SCAN_LIMIT} | merge: cosine>=${COSINE_MERGE} & text>=${MERGE_TEXT} | cluster: cosine>=${COSINE_CLUSTER} | date<=${DAYS}d geo<=${KM}km`)

  // 1. Scan sample: approved reports (optionally one category).
  ...
  for (const R of scan) {
    ...
    for (const n of neighbors) {
      ...
      let cls: 'MERGE' | 'CLUSTER' | null = null
      if (cosine >= COSINE_MERGE && textId >= MERGE_TEXT) cls = 'MERGE'
      else cls = 'CLUSTER'  // semantically + date/geo aligned but textually distinct → corroboration
      if (!cls) continue

      pairs.push({
        cls, cosine: +cosine.toFixed(4), textId: +textId.toFixed(3), dateGap: gap === null ? null : +gap.toFixed(1),
  ...
  const show = (label: string, list: Pair[]) => {
    ...
    for (const p of ordered.slice(0, EXAMPLES)) {
      console.log(`\n[${p.cls}] cosine=${p.cosine} textId=${p.textId} dateGap=${p.dateGap}d km=${p.km} ${p.crossSource ? 'CROSS-SOURCE' : 'same-source'}`)
  ...
  fs.writeFileSync(OUT, JSON.stringify({ params: { CATEGORY, SCAN_LIMIT, COSINE_MERGE, COSINE_CLUSTER, MERGE_TEXT, DAYS, KM }, merges, clusters }, null, 2))
