 ...
 *
 * V11.17.40 — Auto-targets merge for the 3 previously-unsupported categories
 *   (ufos_aliens, ghosts_hauntings, cryptids). When any of these categories
 *   is selected via --category / --all / --phenomena, the script loads
 *   src/lib/ingestion/utils/auto-targets.json and merges those entries
 *   into the in-memory TARGETS map. The auto-generated entries cover all
 *   392 active phens across the 3 categories with keywords derived from
 *   phenomena.name / slug / aliases / quick_facts.also_known_as and a
 *   generic 3-rule evidence template. Re-generate the JSON via
 *   scripts/generate-auto-targets.ts. Hand-curated TARGETS for the other
 *   5 categories are unaffected.
 *
...
// ─────────────────────────────────────────────────────────────────────
// V11.17.40 — AUTO-TARGETS MERGE
// ─────────────────────────────────────────────────────────────────────
//
// For ufos_aliens / ghosts_hauntings / cryptids — the 3 categories not
// hand-curated above — we load auto-targets.json (generated by
// scripts/generate-auto-targets.ts) and graft its entries into TARGETS.
// Hand-curated categories (consciousness_practices, perception_sensory,
// psychic_phenomena, religion_mythology, esoteric_practices,
// psychological_experiences) take precedence over auto-gen for any slug
// that happens to appear in both (none do today; the categories are
// disjoint, but the precedence is defensive in case a phen gets
// re-categorized later).

const AUTO_TARGETS_PATH = path.resolve(process.cwd(), 'src/lib/ingestion/utils/auto-targets.json');
const AUTO_TARGETS_CATEGORIES = new Set(['ufos_aliens', 'ghosts_hauntings', 'cryptids']);

function mergeAutoTargets(): void {
  if (!fs.existsSync(AUTO_TARGETS_PATH)) {
    console.warn('auto-targets.json missing — skipping auto-merge (hand-curated TARGETS only)');
    return;
  }
  let payload: any;
  try {
    payload = JSON.parse(fs.readFileSync(AUTO_TARGETS_PATH, 'utf-8'));
  } catch (e) {
    console.warn('auto-targets.json failed to parse — skipping: ' + (e as Error).message);
    return;
  }
  const targets = payload?.targets;
  if (!targets || typeof targets !== 'object') {
    console.warn('auto-targets.json has no .targets — skipping');
    return;
  }
  let merged = 0;
  for (const [cat, entries] of Object.entries(targets)) {
    if (!AUTO_TARGETS_CATEGORIES.has(cat)) continue;
    if (!Array.isArray(entries)) continue;
    // Hand-curated take precedence: build slug-set from existing TARGETS[cat],
    // skip auto-entries with same slug.
    const existing = TARGETS[cat] || [];
    const existingSlugs = new Set(existing.map(t => t.slug));
    const additions: PhenomenonTarget[] = [];
    for (const e of entries as any[]) {
      if (!e || typeof e !== 'object') continue;
      if (typeof e.slug !== 'string' || typeof e.name !== 'string') continue;
      if (!Array.isArray(e.keywords) || !Array.isArray(e.evidenceRules)) continue;
      if (existingSlugs.has(e.slug)) continue;
      additions.push({
        slug: e.slug,
        name: e.name,
        keywords: e.keywords.filter((k: any) => typeof k === 'string'),
        evidenceRules: e.evidenceRules.filter((r: any) => typeof r === 'string'),
        description: typeof e.description === 'string' ? e.description : '',
      });
    }
    TARGETS[cat] = [...existing, ...additions];
    merged += additions.length;
  }
  console.log('auto-targets merge: +' + merged + ' phens grafted from auto-targets.json');
}
...
async function findCandidates(
  ...
  // V11.17.40 — bulk-load all phenomena IDs in one query (was per-phen
  // `.eq().single()` × N). With 392-phen auto-gen categories the
  // sequential lookups alone took 15-30s before any keyword work began.
  // One `.in()` query gets them all.
  const slugs = targets.map(t => t.slug);
  const phenIdBySlug = new Map<string, string>();
  for (let i = 0; i < slugs.length; i += 200) {
    const chunk = slugs.slice(i, i + 200);
    const { data: phenRows } = await supabase.from('phenomena').select('id, slug').in('slug', chunk);
    for (const r of (phenRows || [])) phenIdBySlug.set(r.slug, r.id);
  }

  // V11.17.40 — parallelize per-phen candidate-finding with a bounded
  // concurrency pool. The legacy sequential loop took N × keywords ×
  // network-RTT which is ~30-90s for 92+ phens. Empirically Supabase
  // throttles parallel ilike-OR scans hard above ~4 concurrent
  // (single-query ~250ms, 8-parallel ~3s/query, 16-parallel ~5s/query).
  // 6 is a sweet spot: ~2x serial throughput without hitting throttle.
  const PHEN_CONCURRENCY = 6;

  async function findForPhen(target: PhenomenonTarget): Promise<Candidate[]> {
    const phenId = phenIdBySlug.get(target.slug);
    if (!phenId) {
      console.warn('  no phenomena row for slug=' + target.slug + ' — skipping');
      return [];
    }
    const phen = { id: phenId };
    const phenCandidates: Candidate[] = [];
    let perPhenCount = 0;
    for (const keyword of target.keywords) {
      ...
      for (const r of matches) {
        if (perPhenCount >= perPhenLimit) break;
        // V11.17.40 — skip per-candidate DB checks in dry-run mode. With
        // 392-phen auto-gen categories, the sequential already-linked +
        // rejection-memo lookups are the dominant runtime. Apply mode
        // keeps them (they're amortized by Haiku batch latency anyway,
        // and they prevent wasted Haiku spend on already-classified or
    ...
    return phenCandidates;
  }

  // Bounded-concurrency driver over phens. Each task returns its
  // local candidate list; the driver merges into `candidates` with
  // dedup on report id and honors totalLimit.
  let nextIdx = 0;
  let stopped = false;
  const inflight = new Set<Promise<void>>();
  while ((nextIdx < targets.length && !stopped) || inflight.size > 0) {
    while (!stopped && inflight.size < PHEN_CONCURRENCY && nextIdx < targets.length) {
      const t = targets[nextIdx++];
      const p = findForPhen(t).then(found => {
        for (const c of found) {
          if (stopped) return;
          if (seenIds.has(c.id)) continue;
          if (candidates.length >= totalLimit) { stopped = true; return; }
          candidates.push(c);
          seenIds.add(c.id);
        }
      }).catch(err => {
        console.warn('  findForPhen failed for ' + t.slug + ': ' + (err?.message || err));
...
async function processCategory(
  ...
  const candidates = await findCandidates(supabase, category, limit, dryRun);
...
async function main() {
  ...
  // V11.17.40 — merge auto-targets.json (ufos_aliens / ghosts_hauntings /
  // cryptids) into TARGETS before any category routing. Hand-curated
  // entries take precedence; auto entries fill the remainder.
  mergeAutoTargets();
