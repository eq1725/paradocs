#!/usr/bin/env npx tsx
/**
 * SPR Phantasms modernization pass — founder-approved June 11, 2026.
 *
 * For every reports row with source_type='spr':
 *   1. Haiku rewrites the 1886 deposition into a modern documentary-register
 ...
 *      letter/deposition date; must be ≤1886; null when indeterminable.
 ...
 *
 * Usage:
 *   npx tsx scripts/spr-modernize.ts --dry-run --limit 3     # print, no writes
 *   npx tsx scripts/spr-modernize.ts --limit 40              # one chunk
 *   npx tsx scripts/spr-modernize.ts                          # all remaining
 * Flags: --limit N, --dry-run, --concurrency N (default 5), --max-cost USD (default 12), --id spr-phantasms-N
 */
...
const PUBLISHED_CUTOFF = '1886-12-31';

interface CliArgs { dryRun: boolean; limit: number; concurrency: number; maxCostUsd: number; id: string | null; }
...
const SYSTEM_PROMPT = `You are an editor for Paradocs, a documentary archive of anomalous experiences. You retell Victorian-era Society for Psychical Research case depositions for a modern general audience.

Given one case text from "Phantasms of the Living" (1886), output ONLY JSON:

{
  ...
  "body": string,         // 150-400 words. Faithful modern retelling in documentary register. Preserve ALL substantive facts: who (keep names as given, incl. initials/redactions like "Mrs. W."), where, when, what was experienced, physical details, other witnesses, and the corroboration/coincidence (e.g. the death learned of later). Present testimony as testimony: "she wrote", "he reported to the researchers". Where the original hedges, keep the hedge. Quote a short striking phrase from the original verbatim (in quotes) when one exists. Close with one plain sentence situating it: collected and corroborated by the Society for Psychical Research and published in Phantasms of the Living (1886). NEVER invent details. EXCLUDE: footnotes and cross-references (e.g. "Compare cases 659 and 670", "see p. 190"), editorial apparatus, page artifacts, and any trailing text that clearly begins the NEXT case (e.g. "The next case is from...", a new "From Mr. X..." attribution after the narrative has concluded).
  "event_date": string|null,   // YYYY-MM-DD. The date the EXPERIENCE occurred — never the date the letter/deposition was written. "October 1884" at the head of a letter is the LETTER date; if the narrative says the experience was "in June, 1867" or "12 years ago", derive from that. Use -01 for unknown month/day. Must be <= 1886-12-31. null if genuinely indeterminable.
...
async function haiku(caseText: string, apiKey: string): Promise<{ json: any; costUsd: number }> {
...
function validate(out: any): string | null {
  ...
  if (out.event_date != null) {
    ...
    if (out.event_date > PUBLISHED_CUTOFF || out.event_date < '1500-01-01') return `implausible date ${out.event_date}`;
  }
  return null;
}
...
async function main() {
  ...
  console.log(`[spr-modernize] ${rows.length} spr rows; ${rows.filter(r => r.metadata?.modernized === true).length} already done; processing ${todo.length}${args.dryRun ? ' (DRY RUN)' : ''}`);
  ...
  async function processRow(r: any) {
    ...
    try {
      const { json: out, costUsd } = await haiku(r.description, apiKey!);
      cost += costUsd;
  ...
  if (!args.dryRun && cost > 0) {
    ...
    await supabase.from('paradocs_narrative_cost_log').insert({ service: 'spr-modernize', cost_usd: cost, report_id: null, model: MODEL }).then(({ error }) => {
      if (error) console.warn('[spr-modernize] cost log insert failed:', error.message);
    });
  }

  console.log(`\n[spr-modernize] done=${done} failed=${failed} dateChanged=${dateChanged} bleedRemoved=${bleed} cost=$${cost.toFixed(2)}`);
