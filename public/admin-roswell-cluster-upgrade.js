// ═══════════════════════════════════════════════════════════════════════
// ROSWELL CLUSTER — Full Upgrade Script (Phase B: Perfect Reports)
// Run in browser console on beta.discoverparadocs.com while logged in as admin
//
// What this does:
// 1. Fixes category/metadata on existing 4 witness reports
// 2. Adds media (images + external links) to existing witness reports
// 3. Creates 4 new witness reports (Porter, Cavitt, Marcel Jr., Wilcox)
// 4. Links everything via case_group + report_links + phenomenon_type_id
// 5. Invalidates cached AI insights so they regenerate
//
// Safe to run multiple times — uses upsert/skip logic for idempotency
// ═══════════════════════════════════════════════════════════════════════

(async function roswellClusterUpgrade() {
  console.log('\n');
  console.log('==========================================================');
  console.log('  ROSWELL CLUSTER UPGRADE — Phase B: Perfect Reports');
  console.log('==========================================================');
  console.log('');

  // ─── Setup ──────────────────────────────────────────────────────────
  var { createClient } = await import('https://esm.sh/@supabase/supabase-js@2');
  var supabaseUrl = 'https://bhkbctdmwnowfmqpksed.supabase.co';
  var supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJoa2JjdGRtd25vd2ZtcXBrc2VkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njk1MTk4NjIsImV4cCI6MjA4NTA5NTg2Mn0.eQAyAKbNwfmJZzSgGTz1hTH-I5IWYa7E2pLJER6M8bc';
  var sb = createClient(supabaseUrl, supabaseKey);

  var CASE_GROUP = 'roswell-1947';
  var results = { fixed: 0, mediaAdded: 0, reportsCreated: 0, linked: 0, errors: [] };

  // ─── Step 1: Get showcase report and phenomenon_type_id ─────────────
  console.log('[1/6] Finding showcase report...');
  var { data: showcase, error: findErr } = await sb
    .from('reports')
  ...
  if (findErr || !showcase) {
    console.error('FATAL: Showcase report not found. Run seed-showcase first.');
    return;
  }
  console.log('  Found showcase: ' + showcase.id);

  var phenomenonTypeId = showcase.phenomenon_type_id;
  if (!phenomenonTypeId) {
    // Try to find UFO/UAP phenomenon type
    var { data: ufoType } = await sb.from('phenomenon_types')
      .select('id').ilike('name', '%ufo%').limit(1).single();
    phenomenonTypeId = ufoType ? ufoType.id : null;
    console.log('  Looked up phenomenon_type_id: ' + phenomenonTypeId);
  }

  // ─── Step 2: Fix existing witness report metadata ───────────────────
  console.log('\n[2/6] Fixing existing witness report metadata...');
  ...
  if (phenomenonTypeId) {
    fixPayload.phenomenon_type_id = phenomenonTypeId;
  }

  var { data: fixedReports, error: fixErr } = await sb.from('reports')
    .update(fixPayload)
  ...
  if (fixErr) {
    ...
    results.fixed = fixedReports ? fixedReports.length : 0;
    console.log('  Fixed ' + results.fixed + ' existing witness reports');
  }

  // Build slug-to-id map for all existing reports
  var { data: allExisting } = await sb.from('reports')
    .select('id, slug')
    .in('slug', [showcase.id ? 'the-roswell-incident-july-1947-showcase' : ''].concat(existingSlugs));

  // Actually just fetch all roswell reports
  var { data: roswellReports } = await sb.from('reports')
    .select('id, slug')
    .eq('case_group', CASE_GROUP);

  var slugToId = {};
  if (roswellReports) {
    roswellReports.forEach(function(r) { slugToId[r.slug] = r.id; });
  }
  // Ensure showcase is in there
  slugToId['the-roswell-incident-july-1947-showcase'] = showcase.id;

  // Also fetch by slug for any that might not have case_group yet
  var { data: bySlug } = await sb.from('reports')
    .select('id, slug')
    .in('slug', existingSlugs);
  if (bySlug) {
    bySlug.forEach(function(r) { slugToId[r.slug] = r.id; });
  }

  console.log('  Known reports: ' + Object.keys(slugToId).length);

  // ─── Step 3: Add media to existing witness reports ──────────────────
  console.log('\n[3/6] Adding media to existing witness reports...');

  // Media for each witness (images from Wikimedia Commons + external links)
  // Using direct Wikimedia URLs since these are public domain historical photos
  var witnessMedia = {
    'jesse-marcel-roswell-debris-field-1947': [
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/8/8a/Marcel-roswell-debris_0.jpg',
        caption: 'Major Jesse Marcel poses with debris recovered from the Foster Ranch near Roswell. He later claimed this was substituted weather balloon material, not the actual anomalous wreckage he recovered.',
        is_primary: true,
      },
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/a/af/Maj_Jesse_A_Marcel_of_Houma.jpg',
        caption: 'Major Jesse A. Marcel of Houma, Louisiana. As intelligence officer for the 509th Bomb Group, he was the first military person to inspect the debris field on the Foster Ranch.',
        is_primary: false,
      },
      {
        media_type: 'document',
        url: 'https://vault.fbi.gov/Roswell%20UFO/Roswell%20UFO%20Part%2001%20%28Final%29/view',
        caption: 'FBI Vault \u2014 Declassified FBI documents related to the Roswell UFO incident, including internal memos referencing the recovery.',
        is_primary: false,
      },
      {
        media_type: 'video',
        url: 'https://archive.org/details/CSPAN3_20210620_202900_Reel_America_The_Roswell_Reports_-_1997',
        caption: '"The Roswell Reports" (1997) \u2014 Official Air Force documentary on C-SPAN3 covering Project Mogul and crash test dummy explanations.',
        is_primary: false,
      },
    ],
    'mac-brazel-roswell-debris-discovery-1947': [
      {
        ...
        caption: 'Roswell Daily Record, July 8, 1947 \u2014 Front page headline after Brazel reported his find to the sheriff. His discovery on the Foster Ranch triggered the most famous UFO incident in history.',
        is_primary: true,
      },
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/5/5f/Roswell_Crash_1947.jpg',
        caption: 'Photograph associated with the 1947 Roswell crash site area \u2014 the remote New Mexico terrain where Mac Brazel discovered debris scattered across his grazing land.',
        is_primary: false,
      },
      {
        media_type: 'document',
        url: 'https://www.gao.gov/assets/nsiad-95-187.pdf',
        caption: 'GAO Report NSIAD-95-187 \u2014 Results of the search for records concerning the 1947 crash near Roswell. Found that key RAAF administrative records had been destroyed.',
        is_primary: false,
      },
    ],
    'walter-haut-roswell-press-release-1947': [
      {
        ...
        caption: 'The International UFO Museum and Research Center in Roswell, New Mexico \u2014 co-founded in 1991 by Walter Haut himself, the man who issued the original "flying disc" press release.',
        is_primary: true,
      },
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/0/0a/Roswell_Daily_Record._July_8%2C_1947._RAAF_Captures_Flying_Saucer_On_Ranch_in_Roswell_Region.webp',
        caption: 'The Roswell Daily Record headline Haut helped create \u2014 his press release to local media resulted in this front page on July 8, 1947, before the military retraction hours later.',
        is_primary: false,
      },
      {
        media_type: 'video',
        url: 'https://archive.org/details/gov.archives.341-roswell-1',
    ...
    'thomas-dubose-roswell-coverup-testimony-1947': [
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/9/92/Brig_General_Ramey_Roswell_debris.jpg',
        caption: 'Brigadier General Roger Ramey (DuBose\u2019s commanding officer) examines debris in his Fort Worth office. DuBose later confirmed this display was a deliberate substitution \u2014 not the actual Foster Ranch materials.',
        is_primary: true,
      },
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/b/b0/General_Ramey_with_Roswell_Memo.png',
        caption: 'The "Ramey Memo" \u2014 General Ramey holding a document in his hand during the Fort Worth press conference. Researchers have spent decades attempting to decipher its contents.',
        is_primary: false,
      },
      {
        media_type: 'document',
        url: 'https://www.nsa.gov/portals/75/documents/news-features/declassified-documents/ufo/report_af_roswell.pdf',
        caption: 'NSA-hosted declassified U.S. Air Force report on the Roswell incident \u2014 the official 1994 investigation that DuBose\u2019s testimony directly contradicts.',
        is_primary: false,
      },
    ],
  };

  for (var slug in witnessMedia) {
    var reportId = slugToId[slug];
    if (!reportId) {
      console.log('  Skipping media for ' + slug + ' (report not found)');
      continue;
    }

    var mediaItems = witnessMedia[slug];

    // Check existing media count
    var { data: existingMedia, error: mediaCheckErr } = await sb
      .from('report_media')
      .select('id, url')
      .eq('report_id', reportId);

    var existingUrls = new Set((existingMedia || []).map(function(m) { return m.url; }));

    var newMedia = mediaItems.filter(function(m) { return !existingUrls.has(m.url); });

    if (newMedia.length === 0) {
      console.log('  ' + slug.split('-')[0] + ': all media already exists (' + (existingMedia ? existingMedia.length : 0) + ' items)');
      continue;
    }

    for (var i = 0; i < newMedia.length; i++) {
      var item = newMedia[i];
      var { error: insertErr } = await sb.from('report_media').insert({
        report_id: reportId,
      ...
      if (insertErr) {
        console.error('  Media insert error (' + slug + '): ' + insertErr.message);
        results.errors.push('media ' + slug + ': ' + insertErr.message);
      } else {
        results.mediaAdded++;
      }
    }
    console.log('  ' + slug.split('-')[0] + ': added ' + newMedia.length + ' media items');
  }

  // ─── Step 4: Create new witness reports ─────────────────────────────
  console.log('\n[4/6] Creating new witness reports...');

  var newWitnessReports = [
    {
  ...
  for (var r = 0; r < newWitnessReports.length; r++) {
    var report = newWitnessReports[r];

    // Check if already exists
    var { data: existing } = await sb.from('reports')
      .select('id').eq('slug', report.slug).single();

    if (existing) {
      console.log('  Skipped (exists): ' + report.slug);
      slugToId[report.slug] = existing.id;
      continue;
    }

    // Add phenomenon_type_id
    if (phenomenonTypeId) {
      report.phenomenon_type_id = phenomenonTypeId;
    }

    var { data: inserted, error: insertErr } = await sb.from('reports')
      .insert(report).select('id').single();

    if (insertErr) {
      console.error('  Insert error (' + report.slug + '):', insertErr.message);
    ...
    console.log('  Created: ' + report.title.substring(0, 55) + '...');
  }

  // ─── Step 5: Add media to new witness reports ───────────────────────
  console.log('\n[5/6] Adding media to new witness reports...');
    ...
    'robert-porter-roswell-transport-1947': [
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/9/92/Brig_General_Ramey_Roswell_debris.jpg',
        caption: 'The Fort Worth press conference where debris was displayed \u2014 Porter\u2019s B-29 flight transported materials here from Roswell before they were reshipped to Wright Field.',
        is_primary: true,
      },
      {
        media_type: 'video',
        url: 'https://archive.org/details/gov.archives.341-roswell-1',
        caption: 'Roswell Reports, Volume 1 \u2014 National Archives footage covering the official investigation and the transport chain Porter was part of.',
        is_primary: false,
      },
    ],
    'george-wilcox-roswell-sheriff-1947': [
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/0/0a/Roswell_Daily_Record._July_8%2C_1947._RAAF_Captures_Flying_Saucer_On_Ranch_in_Roswell_Region.webp',
        caption: 'Roswell Daily Record, July 8, 1947 \u2014 the chain of events started when Brazel walked into Sheriff Wilcox\u2019s office. Wilcox\u2019s phone call to the base set the entire Roswell incident in motion.',
        is_primary: true,
      },
      {
        media_type: 'document',
        url: 'https://www.gao.gov/assets/nsiad-95-187.pdf',
        caption: 'GAO Report NSIAD-95-187 \u2014 Found that key RAAF administrative records from 1945-1949 had been destroyed, including records that would have documented the sheriff\u2019s initial report.',
        is_primary: false,
      },
    ],
    'jesse-marcel-jr-roswell-debris-1947': [
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/8/8a/Marcel-roswell-debris_0.jpg',
        caption: 'Major Jesse Marcel Sr. with Roswell debris \u2014 Marcel Jr.\u2019s father brought similar material home for his family to see on the night of July 7, 1947, before returning it to the base.',
        is_primary: true,
      },
      {
        media_type: 'video',
        url: 'https://archive.org/details/CSPAN3_20210620_202900_Reel_America_The_Roswell_Reports_-_1997',
        caption: '"The Roswell Reports" (1997) \u2014 Air Force documentary presenting their official explanation, which Marcel Jr. publicly disputed for decades.',
        is_primary: false,
      },
    ],
    'sheridan-cavitt-roswell-cic-1947': [
      {
        media_type: 'image',
        url: 'https://upload.wikimedia.org/wikipedia/commons/5/5f/Roswell_Crash_1947.jpg',
        caption: 'The remote New Mexico terrain near the Foster Ranch \u2014 Cavitt and Marcel traversed this landscape together on July 7, 1947, but would later give drastically different accounts of what they found.',
        is_primary: true,
      },
      {
        media_type: 'document',
        url: 'https://www.nsa.gov/portals/75/documents/news-features/declassified-documents/ufo/report_af_roswell.pdf',
        caption: 'The 1994 Air Force Roswell Report \u2014 Cavitt\u2019s interview for this investigation was the only time he provided a detailed account, claiming to have recognized the debris as a weather balloon immediately.',
        is_primary: false,
      },
    ],
  };

  for (var nSlug in newWitnessMedia) {
    var nReportId = slugToId[nSlug];
    if (!nReportId) {
      console.log('  Skipping media for ' + nSlug + ' (report not found)');
      continue;
    }

    var nMediaItems = newWitnessMedia[nSlug];

    // Check existing media
    var { data: nExistingMedia } = await sb.from('report_media')
      .select('url').eq('report_id', nReportId);

    var nExistingUrls = new Set((nExistingMedia || []).map(function(m) { return m.url; }));
    var nNewMedia = nMediaItems.filter(function(m) { return !nExistingUrls.has(m.url); });

    if (nNewMedia.length === 0) {
      console.log('  ' + nSlug.split('-')[0] + ': media already exists');
      continue;
    }

    for (var j = 0; j < nNewMedia.length; j++) {
      var nItem = nNewMedia[j];
      var { error: nInsertErr } = await sb.from('report_media').insert({
        report_id: nReportId,
        media_type: nItem.media_type,
        url: nItem.url,
        caption: nItem.caption,
        is_primary: nItem.is_primary,
      });

      if (nInsertErr) {
        results.errors.push('media ' + nSlug + ': ' + nInsertErr.message);
      } else {
        results.mediaAdded++;
      }
    }
    console.log('  ' + nSlug.split('-')[0] + ': added ' + nNewMedia.length + ' media items');
  }

  // ─── Step 6: Link everything together ───────────────────────────────
  console.log('\n[6/6] Linking all reports into cluster...');

  // Set case_group on all new reports
  var allNewSlugs = newWitnessReports.map(function(r) { return r.slug; });
  var newReportIds = allNewSlugs
    .map(function(s) { return slugToId[s]; })
    .filter(function(id) { return !!id; });

  if (newReportIds.length > 0) {
    var { error: groupErr } = await sb.from('reports')
      .update({ case_group: CASE_GROUP })
      .in('id', newReportIds);

    if (groupErr) {
      console.error('  case_group error:', groupErr.message);
    } else {
      console.log('  Set case_group on ' + newReportIds.length + ' new reports');
    }
  }

  // Create report_links from showcase to each new witness
  var { data: existingLinks } = await sb.from('report_links')
    .select('source_report_id, target_report_id')
    .eq('source_report_id', showcase.id);

  var linkedTargets = new Set((existingLinks || []).map(function(l) { return l.target_report_id; }));

  var linksToCreate = [];
  for (var ls = 0; ls < allNewSlugs.length; ls++) {
    var tId = slugToId[allNewSlugs[ls]];
    if (tId && !linkedTargets.has(tId)) {
      linksToCreate.push({
        source_report_id: showcase.id,
        target_report_id: tId,
  ...
  if (linksToCreate.length > 0) {
    var { error: linkErr } = await sb.from('report_links').insert(linksToCreate);
    if (linkErr) {
      console.error('  report_links error:', linkErr.message);
      results.errors.push('report_links: ' + linkErr.message);
    } else {
      results.linked = linksToCreate.length;
      console.log('  Created ' + linksToCreate.length + ' new report links');
    }
  } else {
    console.log('  All report links already exist');
  }

  // Invalidate AI insights so they regenerate with new cluster context
  var allIds = Object.values(slugToId);
  var { error: staleErr } = await sb.from('report_insights')
    .update({ is_stale: true })
    .in('report_id', allIds);

  if (!staleErr) {
    console.log('  Invalidated cached AI insights for regeneration');
  }

  // ─── Summary ────────────────────────────────────────────────────────
  console.log('\n');
  console.log('==========================================================');
  console.log('  ROSWELL CLUSTER UPGRADE COMPLETE');
  console.log('==========================================================');
  console.log('  Metadata fixed:    ' + results.fixed + ' existing reports');
  console.log('  Media added:       ' + results.mediaAdded + ' items');
  console.log('  Reports created:   ' + results.reportsCreated + ' new witnesses');
  console.log('  Links created:     ' + results.linked + ' report links');
  console.log('  Total cluster:     ' + Object.keys(slugToId).length + ' reports');
  if (results.errors.length > 0) {
    console.log('  Errors:            ' + results.errors.length);
    results.errors.forEach(function(e) { console.log('    - ' + e); });
  }
  console.log('');
  console.log('  Cluster reports:');
  Object.keys(slugToId).forEach(function(s) {
    console.log('    /report/' + s);
  });
  console.log('');
  console.log('  Refresh any Roswell report page to see the updates.');
  console.log('==========================================================');
